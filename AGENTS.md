...
## Notes projet — Mémoire CUF

**Projet** : Mémoire M2 — Amélioration des performances de la chaîne 4, scierie CUF Ebolowa

**Flux de travail recommandé** :
- `$deep-interview` → clarifier une section avant rédaction
- `$ralplan` → structurer un plan de section
- `/rpi:rediger` → produire le texte final (skill Claude Code)

**Conventions** :
- Toujours ancrer les arguments dans les données terrain CUF (chaîne 4, bicoupe, essences)
- Ne jamais inventer de données terrain — marquer `[DONNÉE MANQUANTE]`
- Style académique français, zéro listes à tirets dans les textes rédigés

**Structure du repo** :
- `documents/` → livrables Word et fiches
- `documents/plans/` → plans de sections (RPI)
- `.omx/` → état OMX (ne pas committer les logs)
