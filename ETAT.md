...
- **Alignement app ↔ mémoire final** (soutenance J-1, après lecture du mémoire `test_2.docx`) :
  - **#1 — 4 profils** (le mémoire §2.5/§3.1.4.2 dit « quatre profils ») : compte `chef@cuf.cm` (Chef Scierie) **supprimé** (base + seed `__init__.py`). Restent Opérateur, **Chef de Production** (`prod`), Direction, Admin. Le **rôle `chef` reste dans le code** (routes, `ROLES_VALIDES`) — seul le compte de démo disparaît. Smoke : bloc « chef » repointé sur `admin` (accès identique), reste vert.
  - **#2 — rendement matière = 31 %** (mémoire Tableau 12 : global 30,6 %). Hypothèses d'import alignées : Ayous 33,8 · Movingui 30,9 · Iroko 30,4 · **Bilinga 23,3** ; « Autre » calibré à 0,265 pour un global de 31,0 %. Vérifié à l'écran Qualité.
  - **#3 — objectif 25 m³/poste + production 14,55 (FAIT côté données)** : `objectif_m3=25` et **capacité calée à 2,473 m³/h** pour une production réelle de **14,55 m³/poste** (mémoire §3.1.1.2). Calibration **neutre sur le TRS** (reste 63,3 %) et sur le rendement (31 %). L'outil d'import fixe lui-même ces paramètres (reproductible). **Reste ouvert (A′)** : l'écran « Production & Objectifs » affiche **70 %** d'atteinte, pas 58 %, car il agrège l'objectif sur **2 postes/jour** (hardcodé) alors que les données ont **3 postes/jour**, et compare le **conforme** (12,52) et non le **débité** (14,55). Correction = modifier `_resume_production` (objectif × nb postes réels ; base débité) — en attente d'accord (touche la logique d'affichage). **Manque à gagner** : ~424 M FCFA app (prix locaux) vs 357 M mémoire (prix export FOB 407k/446k) — écart de **base de prix**, pré-existant, non corrigé.
  - **#4 — Pareto « en FCFA »** (mémoire fig 10) : l'écran Pareto est en minutes → non aligné, en attente.
- **Journaux opérateur complétés SANS invention** (voie choisie par l'utilisateur) : les 95 quarts réels restent remplis (verrouillés) ; les **16 quarts manquants** de la période (5 mai–10 juin, trous dans la grille jour×créneau) sont créés en **brouillon vide** (`operateur_nom='À compléter'`), avec en note les essences vues ce jour (indice, source base production). Les brouillons sont **exclus de tous les KPI** → TRS/Pareto inchangés (95 postes comptés). L'utilisateur complètera lui-même. Outil versionné idempotent : `documents/outils/completer_brouillons.py` (à lancer APRÈS l'import).
...
|---|---|---|
| 2026-07-14 | **4 profils (aligné mémoire)** | Supersede la coexistence chef+prod du 09/06 **pour la démo** : le mémoire dit « quatre profils ». Compte `chef@cuf.cm` supprimé (base + seed). Le **rôle `chef` reste dans le code** — ne pas le retirer. Profils démontrables : Opérateur, Chef de Production (`prod`), Direction, Admin. |
| 2026-07-14 | **Rendement matière = 31 %** | Valeurs mémoire Tableau 12 (Ayous 33,8 · Movingui 30,9 · Iroko 30,4 · Bilinga 23,3 ; Autre 0,265 → global 31,0 %). Câblé dans l'outil d'import. |
